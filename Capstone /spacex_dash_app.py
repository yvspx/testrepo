...
#Create list of launch site options 
launch_sites = spacex_df[['Launch Site']].rename(columns={'Launch Site':'label'}).drop_duplicates()
launch_sites['value'] = launch_sites['label']
launch_sites = pd.concat([launch_sites,pd.DataFrame(data={'label':['All Sites'],'value':['ALL']})])
options_dict = launch_sites.to_dict('records') 
...
def get_scatter_chart(entered_site,payload_range):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.scatter(scatter_df, x='Payload Mass (kg)', y='class',
                        color="Booster Version Category",
                         title=f'Payload vs. Success at {entered_site}')
    return fig
